...
- `agent_model_map` 明确记录每个 Agent 的 preferred provider、preferred model 和选择理由。
...
- 默认 mode 是 `dry_run`。
- 不需要 API key。
- `manimax` 与 `anthropic` provider lane 已配置 base URL、环境变量名、Anthropic Messages API 兼容方式、非流式调用约束。
- 非 mock live provider 在 v1 中只是安全占位，默认不会发起真实调用。
