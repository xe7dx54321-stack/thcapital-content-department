...
P0-005 建立了统一 source registry 的第一版配置和读取/校验能力。现有抓取主链路尚未切换到 registry；下一步 P0-006 将基于 registry 增加 source health v1，用于统计各信源运行状态和稳定性。
...
目标是基于 `config/sources.yaml` 建立 source health report，统计每个 source 的 enabled/tier/category、最近运行状态、预期频率和缺失情况。先做静态 health/coverage 报告，不改现有 fetcher。
