...
connector-reliability-improvement:
	$(PYTHON) scripts/improve_p0_connector_reliability.py

connector-evidence-enrichment:
	$(PYTHON) scripts/enrich_evidence_from_connector_items.py

promote-hot-materials-to-topics:
	$(PYTHON) scripts/promote_hot_materials_to_topics.py

connector-freshness-dedup-regression:
	$(PYTHON) scripts/run_connector_freshness_dedup_regression.py

acquisition-to-content-bridge:
	$(PYTHON) scripts/build_acquisition_to_content_bridge.py

phase28-daily:
	$(PYTHON) scripts/run_phase28_daily_enrichment_pipeline.py

openclaw-source-inventory:
	$(PYTHON) scripts/import_openclaw_source_inventory.py

openclaw-source-risk-classification:
	$(PYTHON) scripts/classify_openclaw_source_risk.py

openclaw-migration-plan:
	$(PYTHON) scripts/build_openclaw_migration_plan.py

openclaw-metadata-connectors:
	$(PYTHON) scripts/run_openclaw_metadata_connectors.py

weak-signal-safety-gate:
	$(PYTHON) scripts/run_weak_signal_safety_gate.py

normalize-openclaw-signals:
	$(PYTHON) scripts/normalize_openclaw_signals.py

phase29-daily:
	$(PYTHON) scripts/run_phase29_daily_migration_pipeline.py

openclaw-signal-evidence-backfill:
	$(PYTHON) scripts/backfill_openclaw_signal_evidence.py

weak-signal-confirmation-workflow:
	$(PYTHON) scripts/build_weak_signal_confirmation_workflow.py

openclaw-topic-activation:
	$(PYTHON) scripts/activate_openclaw_migrated_topics.py

openclaw-to-content-regression-gate:
	$(PYTHON) scripts/run_openclaw_to_content_regression_gate.py

openclaw-source-registry-proposal:
	$(PYTHON) scripts/build_openclaw_source_registry_proposal.py
