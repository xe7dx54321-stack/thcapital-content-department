...
- `top20_count`: `20`
- `notes`: `本轮新增5个packet；资产链从52个今日packet中派生3条新链；qbitai_site source id报错（修正为qbituai_site待下次抓取）；本包覆盖技术扩散层、Builder层、研究层、中文传播层与YC融资层`
...
### 1. Qwen3.6-27B：旗舰级编程性能，27B dense 模型刷新开源 SOTA

- `topic_key`: `qwen3_6_27b_coding_sota`
- `title`: Qwen3.6-27B: Flagship-Level Coding in a 27B Dense Model
- `primary_platform`: HN / GitHub
- `published_at`: `2026-04-23`
- `original_link`: `https://news.ycombinator.com/item?id=47863217`
- `score_total`: **21/30**
- `score_breakdown`: 一手性=3(官方/开源发布) / 传播性=3(HN前排+多渠道) / 破圈性=2(HN+GitHub) / 赛道匹配=3(模型+编程) / 可延展性=3(评测/对比/应用) / 数据硬度=2(GitHub benchmark截图) / 视觉素材=2(性能图表) / 平台适配=2 / 时效窗口=3 / 讨论度=2
- `signal_summary`: 阿里 Qwen 团队发布 27B dense 模型，编程能力自称对标旗舰级封闭模型。HN 当天冲上前排，GitHub 有 GGUF 权重可下载，社区已出现 unsloth 微调实验帖。
- `why_in_top20`: 开源小模型编程 SOTA 级别事件，builder 圈直接可用的硬货，一手 benchmark 数据 + 社区实测两条硬证据链，时效窗口 48-72 小时内最强。
- `visual_assets`: benchmark 截图、GitHub README 性能对比表、HN 评论截图
- `risks`: 评测数据来自官方博客，需要更多第三方独立验证；dense vs MoE 对比仍有争议空间

---

### 2. Anthropic Mythos 遭非法访问：顶级模型泄露事件的多层解读

- `topic_key`: `anthropic_mythos_illegal_access`
- `title`: Anthropic 顶级模型 Mythos 遭非法访问 / Unauthorized group gained access to Mythos
- `primary_platform`: 36kr / TechCrunch
- `published_at`: `2026-04-23`
- `original_link`: `https://36kr.com/ai` / `https://techcrunch.com/2026/04/22/`
- `score_total`: **20/30**
- `score_breakdown`: 一手性=2(36kr报道+TC跟进) / 传播性=3(中美双线+安全圈) / 破圈性=3(AI安全+商业+政策) / 赛道匹配=3(模型安全) / 可延展性=3(事件复盘+安全解读+公司影响) / 数据硬度=2(报告引用) / 视觉素材=1 / 平台适配=3 / 时效窗口=3 / 讨论度=3
- `signal_summary`: Anthropic 新推出 Mythos 模型（网络安全专用，限制性发布）被未授权组织获取。TC 和 36kr 均有报道，NSA 已在使用 Pentagon 却有分歧，争议持续发酵。
- `why_in_top20`: 高热 + 高争议 + 高可解读性，多层内容角度：安全事件本身、公司战略（限制发布动机）、政策博弈（NSA vs Pentagon）、行业信任影响。
- `visual_assets`: TC 文章截图、36kr 标题截图
- `risks`: 具体技术细节不透明，官方确认信息有限，不宜过度推测
...
### top3_must_watch
1. **Qwen3.6-27B** — 开源小模型编程 SOTA，一手性强，builder 圈即用，48h 时效窗口最强
2. **Anthropic + Amazon $5B** — 大厂 AI 基础设施里程碑，$100B 支出承诺是硬数据，内容多角度
3. **Anthropic Mythos 泄露** — 高热+高争议+多层内容角度，TC+36kr 双验证

### top6_strong_pool
4. Cursor $50B 估值融资（TechCrunch，VC 叙事高热）
5. 小模型才是 Agent 核心组件（中文专业媒体，趋势印证）
6. Claude Code 泄露 + OpenAI 退出视频（The Batch 四合一打包，覆盖全）
7. LinkedIn Cognitive Memory Agent（Agent infra 工程实践，大厂生产系统）
8. Google Cloud 新 AI 芯片（infra 赛道，与 Nvidia 竞争）
9. SpaceX + Cursor $60B 收购期权（大厂+独角兽联动）

### holdout_watchlist
10. 普渡机器人 10 亿人民币融资（中文硬科技稀缺案例）
11. AI Scientists 无科学推理能力（arXiv 学术争议，AI4Science 主线）
12. Karpathy "English is hottest programming language"（高传播金句）
13. Gemini CLI Subagents（Google 开发者工具更新）
14. 宇视 SOP 智能体全家桶（toB Agent 中国落地）
15. 荣耀 Honor Agent（手机端侧 Agent 新赛道）
16. AI 音乐 44% 由 AI 生成（AI 内容消费争议话题）
17. Tesla Robotaxi 达拉斯+休斯顿（自动驾驶商业化）
18. Cerebras IPO（AI 芯片上市潮）
19. AI 卖铲人下场做漫剧（中国 AI 应用层迁移）
20. GPT-5 Latent Space 评测（开发者圈关注，"Stone Age"标题有传播力）

### supply_risk
- 本轮 5 个新 packet 中，3 个来自 36kr，1 个来自 HN，1 个来自 YC；整体覆盖广度较好但部分信源（如 simon_willison、latent_space、interconnects）今日无新增
- `web__qbituai_site` source id 报错，需修正后下次补抓
- Asset derivation 本轮仅产出 3 条新链，部分 packet 判定为"no clear company/project signal"跳过了派生，候选覆盖仍有盲区
- 今日 HN 和 GitHub trending 新增偏少，可能内容发布节奏原因，建议傍晚再跑一轮补充
