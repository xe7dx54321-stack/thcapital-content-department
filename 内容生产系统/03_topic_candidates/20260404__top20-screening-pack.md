...
- `generated_at`: `2026-04-04 14:40 CST`
...
- `visual_assets`: `HN原帖截图+评论区截图 / 量子位微信封面图 / 官方邮件全文截图 / Peter推文`
- `risks`: `Reddit评论数不完整（API 403），需补HN评论区截图；Peter推文需确认原始链接`
...
### 4. 谷歌开源Gemma 4——Arena全球第三、超越20倍体量Qwen3.5、Apache 2.0可商用

- `topic_key`: `google-gemma-4-open-source-outperforms-qwen3.5-2026`
...
- `score_total`: `23/30`
- `score_breakdown`: `一手性=2|传播性=2|破圈性=2|赛道匹配=3|可延展性=2|数据硬度=2|视觉素材=2|平台适配=3|时效窗口=2|讨论度=3`
- `signal_summary`: `谷歌本周开源Gemma 4系列，基于Gemini 3研究，在Arena AI排行榜拿全球第三，超越参数量大20倍的模型。支持25.6万token上下文，140+语言，Apache 2.0开源许可证。四种规模：E2B/E4B/26B A4B/31B。密集型+MoE混合架构，覆盖手机到服务器全场景。`
- `why_in_top20`: `①Gemma 4 vs Qwen3.5是中美开源模型竞争强叙事；②Arena第三+超越20倍体量=技术亮点明确；③Apache 2.0可商用是商业化强信号；④中文/英文双圈同时讨论；⑤谷歌开源节奏持续加码`
- `visual_assets`: `Arena排行榜截图 / 机器之心封面图 / Gemma 4技术架构图`
- `risks`: `Deep article正文较短（仅496字），需补Gemma 4官方博客或GitHub页面；Arena排行榜截图需补充`
...
### 6. Anthropic二级市场大热——Private share最热交易、SpaceX IPO阴影下承压
...
### 7. Netflix首发HuggingFace公开模型VOID——视频目标交互删除，GitHub已开源
...
- `signal_summary`: `Netflix在HuggingFace发布首个公开模型VOID（Video Object and Interaction Deletion），用于视频编辑中的对象删除与交互检测。GitHub已开源，有Demo页面。这是Netflix首次在HuggingFace发布正式开源模型，Reddit LocalLLaMA日榜第1。`
- `why_in_top20`: `①Netflix首次发布HF模型=大厂开源新动态；②GitHub+HuggingFace+Demo全链路开源；③视频AI编辑是AIGC重要方向；④Reddit高热（LocalLLaMA第1）`
- `visual_assets`: `HuggingFace模型页面 / VOID Demo截图 / GitHub repo截图`
- `risks`: `Reddit API被403，缺少评论数；VOID技术细节需补充HF页面内容`

---

### 8. 微软一口气发布三款自研AI模型——独立于OpenAI的战略信号
...
- `source_packet`: `/Users/apple/Documents/同行资本内容部门/内容生产系统/02_topic_radar/source_packets/20260403_222304__wechat_zhidx_openai_ai__source-packet.md`
- `score_total`: `21/30`
...
- `risks`: `source packet较简，需补微软官方博客或产品页面；实测数据需补原文`

---

### 9. Kimi推「期权时光机」截胡顶尖大脑——实习生争夺战与AI人才定价
...
- `source_packet`: `/Users/apple/Documents/同行资本内容部门/内容生产系统/02_topic_radar/source_packets/_224533__jiqizhixin_site_kimi__source-packet.md`
- `deep_article`: `/Users/apple/Documents/同行资本内容部门/内容生产系统/02_topic_radar/deep_articles/20260404_121435__天才实习生看过来_kimi要用_期权时光机_截胡你的顶尖大脑__deep-article.md`
- `score_total`: `19/30`
- `score_breakdown`: `一手性=2|传播性=1|破圈性=1|赛道匹配=2|可延展性=2|数据硬度=2|视觉素材=1|平台适配=3|时效窗口=3|讨论度=2`
- `signal_summary`: `Kimi推出"期权时光机"人才计划，旨在以特殊期权结构争夺顶尖AI实习生。机器之心和量子位同步报道。`
- `why_in_top20`: `①"期权时光机"命名强、新颖；②AI人才争夺是持续热点；③与OpenAI/Anthropic扩张形成人才对比叙事；④实习生视角切入独特`
- `visual_assets`: `机器之心封面图`
- `risks`: `source packet为快照层，原文详情未展开；需补Kimi官方公告或完整微信稿`

---

### 10. OpenAI完成1220亿美元融资——史上最大规模，但二级市场"没人接盘"

- `topic_key`: `openai-122b-funding-valuation-private-market-no-buyer-2026`
- `title`: `OpenAI刚融到1220亿美元，却在二级市场"没人接盘"，马斯克表示"毫不意外"`
- `primary_platform`: `zhihu`
- `published_at`: `2026-04-03 17:16 CST`
- `original_link`: `知乎热榜`
- `source_packet`: `/Users/apple/Documents/同行资本内容部门/内容生产系统/02_topic_radar/source_packets/20260403_171613__zhihu_hot_ai_openai_1220__source-packet.md`
- `score_total`: `20/30`
- `score_breakdown`: `一手性=1|传播性=2|破圈性=3|赛道匹配=3|可延展性=3|数据硬度=2|视觉素材=1|平台适配=2|时效窗口=2|讨论度=3`
- `signal_summary`: `OpenAI完成史上规模最大1220亿美元融资，但二级市场出现"没人接盘"现象。知乎热榜，马斯克表态"毫不意外"，引发讨论。`
- `why_in_top20`: `①$122B是历史级融资数字；②二级市场接盘难=估值见顶争议；③马斯克表态增强戏剧性；④知乎+微信双圈传播`
- `visual_assets`: `知乎热榜截图`
- `risks`: `知乎热榜条目较简；需补$1220亿原始融资公告；二级市场接盘现状需补充具体数据`
...
### 12. Anthropic为用户补贴一个月订阅费——"薅羊毛"争议与用户反馈
...
### 13. CVPR 2026 | ReLaX从隐空间动力学重新解读大模型RL探索
...
### 16. 斯坦福研究：ChatGPT骗了你，你却用五星好评杀死了诚实的AI
...
### 17. YC Complir——帮助大零售商消除合规瓶颈的合规科技
...
### 20. qwen 3.6 voting——社区对Qwen 3.6模型的投票讨论
...
- `top3_must_watch`:
  1. **Anthropic封杀OpenClaw + 龙虾之父回应** (score=26/30 | topic_key=anthropic-openclaw-block-third-party-harness-2026)——多平台爆发、即时性最强、叙事完整、龙虾之父IP强
...
- `top6_strong_pool`:
  4. **谷歌开源Gemma 4干掉Qwen3.5** (score=23/30 | topic_key=google-gemma-4-open-source-outperforms-qwen3.5-2026)——中美开源竞争叙事、Arena数据强
  5. **微软三款自研AI模型** (score=21/30 | topic_key=microsoft-releases-three-own-ai-models-bypassing-openai-2026)——微软vs OpenAI战略分化、实测角度好
  6. **Netflix VOID模型** (score=20/30 | topic_key=netflix-void-model-huggingface-open-source-2026)——Netflix首开源、视频AI编辑方向、GitHub/HF全链路
  7. **Anthropic 4000万收购Coefficient Bio** (score=20/30 | topic_key=anthropic-acquires-coefficient-bio-400m-2026)——AI+biotech大额并购
  8. **OpenAI 1220亿美元融资二级市场接盘难** (score=20/30 | topic_key=openai-122b-funding-valuation-private-market-no-buyer-2026)——历史级融资数字、估值争议、马斯克表态
  9. **Kimi期权时光机** (score=19/30 | topic_key=kimi-stock-option-time-machine-talent-war-2026)——人才争夺叙事强、命名新颖

- `holdout_watchlist`:
  - Anthropic二级市场最热 (score=19/30 | topic_key=anthropic-private-market-moment-spacex-ipo-2026)——与#5/#7可合并为"Anthropic商业化扩张"主题
  - AI视频"偷脸"《桃花簪》下架 (score=18/30)——法律/监管角度，差异化
  - Anthropic用户补贴一个月订阅 (score=17/30)——与#1构成戏剧性对比
  - 美团原生多模态 (score=17/30)——技术方向独特，美团大厂动态
...
- `supply_risk`: `本业务日窗口共收录86个source packet，高质量（score ≥20）候选共8个。候选题一手来源占比中等，部分候选仍需补官方链接/论文原文。视觉素材丰富度中等，微信文章和TechCrunch覆盖较完整，Reddit/HN评论数因API限制不完整。`

- `memo_for_editor`: `本包不包含任何morning_flash车道已发布或已排队对象，严格隔离day_mainline单一车道。14:25:30为最晚有效capture，窗口准时关闭。#1 Anthropic/OpenClaw事件于14:00 PT / 4月4日正式生效，即时性极强，建议优先处理。`
