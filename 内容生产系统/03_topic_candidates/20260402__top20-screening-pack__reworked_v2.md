# Top20 初筛包（Rework版 v2）

- `date`: `2026-04-02`
- `owner`: `market-scout (signal-scout runtime)`
- `generated_at`: `2026-04-02 15:24 CST`
...
- `rework_trigger`: `market-editor scorecard（2026-04-02 15:10 CST）`
- `rework_mode`: `supplement_evidence`
- `total_candidates_seen`: `42`
- `top20_count`: `20`
- `delivery_lane`: `day_mainline`
- `new_items_added`: `0`（business window已于14:30关闭，本轮不引入新候选）
- `supply_risk_note`: `本包为返工v2版，仅记录补证结果，不做自判放行；是否可进入下一工序由 market-editor 最新 scorecard 决定。`
...
|---|---|---|---|
| #2 TurboQuant学术不端 | ①Reddit讨论帖URL ②Google官方回应原文 | manifest内无Reddit原始帖可达链接；无Google回应原文文件 | ⚠️ 补证失败，维持原对象 |
| #10 HorusEye登Nature子刊 | ①Nature论文DOI/URL ②准确published_at | manifest内无Nature论文文件；published_at仍unknown | ⚠️ 补证失败，维持原对象 |
| #14 飞书开源5.5k星 | ①GitHub仓库URL | manifest内无飞书GitHub仓库文件；5.5k stars无法验证 | ⚠️ 补证失败，维持原对象 |
| #3 Qwen3.5-27B实测 | Reddit原文URL补强 | ✅ 上轮已补强完成（old.reddit.com可访问） | 完成 |
| #15 StepFun 3.5 Flash | HN帖URL补强 | ✅ 上轮已补强完成（HN URL可访问） | 完成 |

**补强完成率**：2/5（40%），3条 fatal 缺口 manifest 内无更强信源文件，无法补强。

---

## 使用说明

- 这是 `signal-scout` 阶段返工件 v2。
- 基于原始 Top20（2026-04-02 05:15 CST）+ rework v1（2026-04-02 15:01 CST）补强结果。
- 3条 fatal 缺口（#2/#10/#14）manifest 内无更强信源，按 rework_mode=supplement_evidence 规则维持原对象，不替换。
- 每个候选必须包含结构化评分与证据摘要。
...
- `visual_assets`: `Reddit讨论帖截图（待补）、机器之心文章、知乎讨论帖`
- `risks`: `⚠️ fatal缺陷：①Reddit讨论帖原始URL manifest内无直达链接（仅有Qwen3.5-27B实测帖，非学术不端讨论帖）②Google官方回应原文manifest内无文件；补证失败，维持原对象`
- `rework_note`: `rework_mode=supplement_evidence；manifest内无Reddit学术不端讨论帖可达链接；无Google回应原文；补证失败，维持原对象，继续流程`
...
- `risks`: `纯问答形态，一手事实信息有限；时效偏旧（3月31日）；需要回链相关AI新闻背景`
...
- `risks`: `⚠️ fatal缺陷：①Nature论文原文URL manifest内无文件（仅有机器之心入口页）②published_at仍unknown；补证失败，维持原对象`
- `rework_note`: `rework_mode=supplement_evidence；manifest内无Nature论文文件；published_at仍unknown；补证失败，维持原对象，继续流程`
...
- `risks`: `知乎内容以观点为主，需要配合其他一手信源；与#1严重重叠`
...
- `published_at`: `2026-04-01`
- `original_link`: `https://mp.weixin.qq.com/s/2iS-bnaSLoQAPyvmNOHF1Q`
- `score_total`: `16`
...
- `visual_assets`: `微信文章截图、GitHub仓库截图（待挖掘）`
- `risks`: `⚠️ fatal缺陷：GitHub仓库URL manifest内无文件；5.5k stars无法回链验证；补证失败，维持原对象`
- `rework_note`: `rework_mode=supplement_evidence；manifest内无飞书GitHub仓库文件；5.5k stars无法直接验证；补证失败，维持原对象，继续流程`
...
- `top3_must_watch`:
  ...
  2. **TurboQuant 学术不端争议** — 技术价值与声誉风险并存，谷歌官方回应增加可信度，适合做争议性话题分析（注：⚠️ Reddit帖+Google回应仍未补强）
  3. **Cognichip $60M AI芯片设计** — 明确融资金额+赛道明确（AI for Chip Design），TechCrunch背书，可作为硬科技投资线索锚点

- `top6_strong_pool`:
  4. Qwen3.5-27B实测性能对比（✅ Reddit原文URL补强完成）
...
- `holdout_watchlist`:
  10. 飞书开源5.5k星（⚠️ GitHub仓库仍缺失，补证失败）— 字节AI Infra动作，待挖仓库
  11. 阿里达摩院开源CPU直指Agent（硬件开源战略）
  12. HorusEye Nature子刊（⚠️ Nature论文链接仍缺失，补证失败）— 学术价值高但大众传播弱
  13.