#!/usr/bin/env python3
"""Run the Claude live pilot readiness check for llm_critic_agent."""
...
@dataclass(frozen=True)
class ClaudeCriticLivePilotReport:
    schema_version: str
    generated_at: str
    run_date: str
    status: str
...
def build_ready_checks() -> ReadyChecks:
    ...
    return ReadyChecks(
        ...
        adapter_supported=bool(provider and provider.adapter_type == "anthropic_messages"),
    )
...
def readiness_warnings(checks: ReadyChecks) -> tuple[str, ...]:
    ...
    if not checks.provider_configured:
        warnings.append("Anthropic provider is not configured.")
    if not checks.provider_supports_live:
        warnings.append("Anthropic provider does not support live mode in config.")
    ...
    if not checks.agent_allowlisted:
        warnings.append("llm_critic_agent is not in THCAP_LLM_LIVE_AGENT_ALLOWLIST.")
    if not checks.api_key_present:
        warnings.append("ANTHROPIC_API_KEY is not set.")
    if not checks.adapter_supported:
        warnings.append("Anthropic adapter_type is not anthropic_messages.")
...
def output_paths(run_date: str) -> dict[str, Path]:
    ...
    return {
        "dated_json": paths.logs_root / f"{run_date}__claude-critic-live-pilot.json",
        "dated_md": paths.logs_root / f"{run_date}__claude-critic-live-pilot.md",
        "latest_json": paths.logs_root / "latest_claude_critic_live_pilot.json",
        "latest_md": paths.logs_root / "latest_claude_critic_live_pilot.md",
    }
...
def latest_critic_payload() -> dict[str, Any]:
    paths = get_project_paths(REPO_ROOT)
    return read_json(paths.market_content_root / "06_review_queue" / "latest_llm_critic_reviews.json")


def summarize_critic_payload(payload: dict[str, Any]) -> dict[str, Any]:
...
def write_report(report: ClaudeCriticLivePilotReport) -> dict[str, Path]:
...
def build_report(
    *,
    ...
    warnings: tuple[str, ...] = (),
) -> ClaudeCriticLivePilotReport:
    summary = critic_summary or {}
    outputs = output_paths(today_token())
    return ClaudeCriticLivePilotReport(
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run Claude live pilot for llm_critic_agent.")
    ...
    if args.dry_run_check:
        ...
        payload = latest_critic_payload()
        summary = summarize_critic_payload(payload)
    ...
    if args.json:
        ...
        print("Claude Critic Live Pilot")
        print("========================")
